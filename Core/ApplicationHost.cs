...
{
    /// <summary>
    /// Manages the application lifecycle and dependency injection container.
    /// </summary>
    internal class ApplicationHost(IServiceProvider serviceProvider) : IDisposable
    {
        ...
        public Task InitializeAsync()
        {
            ...
            // Start MainController if enabled
            ...
            // Sync proxy state with registry and config
            // If proxy is enabled in config, enable it
            // If proxy is disabled in config but enabled in registry (from previous run), disable it
            ...
            {
                // Proxy is enabled in registry but disabled in config - clean up from previous run
        ...
        public NotifyIcon CreateTrayIcon()
        {
            ...
            if (trayMenu.Items["NF"] is ToolStripMenuItem nfItem)
            {
                nfItem.Click += (s, e) =>
                {
                    var task = Task.Run(async () => await ToggleNetFilterAsync(), _shutdownCts.Token);
                    _backgroundTasks.Add(task);
                };
            }
        ...
        private async Task ToggleNetFilterAsync()
        {
            ...
            {
                _logger.LogError(ex, "Failed to toggle OmniPoss service");
                throw;
            }
        }
        ...
        private void UpdateTrayMenu(ApplicationConfig newConfig)
        {
            ...
            // Update NetFilter and Proxy states are handled in their respective apply methods
        }
        ...
        public async Task CleanUpAsync()
        {
            try
            {
                ...
                try
                {
                    await Task.WhenAll([.. _backgroundTasks]).WaitAsync(TimeSpan.FromSeconds(5));
                }
